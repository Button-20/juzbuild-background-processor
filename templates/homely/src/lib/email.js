...
class EmailService {
    ...
    static async sendConfirmationEmail(data) {
        ...
        const htmlContent = this.generateConfirmationEmailHtml(data, isPropertyInquiry);
        const textContent = this.generateConfirmationEmailText(data, isPropertyInquiry);
    ...
    static async sendNotificationEmail(data) {
        ...
        const htmlContent = this.generateNotificationEmailHtml(data, isPropertyInquiry);
        const textContent = this.generateNotificationEmailText(data, isPropertyInquiry);
        await resend.emails.send({
            from: `${this.config.fromName} <${this.config.fromEmail}>`,
     ...
     */
    static generateConfirmationEmailHtml(data, isPropertyInquiry) {
        return `
<!DOCTYPE html>
<html lang="en">
<head>
...
<body>
    <div class="header">
        ...
        <p>Thank you for ${isPropertyInquiry ? "your property inquiry" : "contacting us"}!</p>
    </div>
    <div class="content">
        <p>Dear ${data.name},</p>
        
        <p>Thank you for ${isPropertyInquiry
            ? "your interest in our property"
            : "reaching out to us"}. We have received your ${isPropertyInquiry ? "inquiry" : "message"} and our team will get back to you within 24 hours.</p>
    ...
    <div class="footer">
        <p>${this.config.companyName} | ${process.env.NEXT_PUBLIC_ADDRESS || ""}</p>
     ...
     */
    static generateConfirmationEmailText(data, isPropertyInquiry) {
        return `
Dear ${data.name},

Thank you for ${isPropertyInquiry ? "your interest in our property" : "contacting us"}. We have received your ${isPropertyInquiry ? "inquiry" : "message"} and our team will get back to you within 24 hours.
...
This is an automated message. Please do not reply to this email.
     ...
     */
    static generateNotificationEmailHtml(data, isPropertyInquiry) {
        return `
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>New ${isPropertyInquiry ? "Property Inquiry" : "Contact"} - ${this.config.companyName}</title>
    <style>
        body { font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px; }
...
<body>
    <div class="header">
        <h1>🔔 New ${isPropertyInquiry ? "Property Inquiry" : "Contact Form Submission"}</h1>
        <p>You have received a new ${isPropertyInquiry ? "property inquiry" : "contact form submission"}</p>
    </div>
    <div class="content">
     ...
     */
    static generateNotificationEmailText(data, isPropertyInquiry) {
        return `
