...
class BlogService {
    ...
    static async create(data) {
        const validatedData = Blog_1.CreateBlogSchema.parse(data);
    ...
    static async findById(id) {
        const collection = await this.getCollection();
        const blog = await collection.findOne({ _id: new mongodb_2.ObjectId(id) });
        return blog || null;
    }
    // Find blog by slug
    ...
    static async update(id, data) {
        const validatedData = Blog_1.UpdateBlogSchema.parse(data);
        const collection = await this.getCollection();
        ...
        const result = await collection.findOneAndUpdate({ _id: new mongodb_2.ObjectId(id) }, { $set: updateData }, { returnDocument: "after" });
        return result || null;
    }
    // Delete blog
    ...
    static async findAll(filters = {}) {
        const validatedFilters = Blog_1.BlogFiltersSchema.parse({
            limit: 10,
            skip: 0,
            ...filters,
        });
    ...
    static async incrementViews(id) {
        const collection = await this.getCollection();
        await collection.updateOne({ _id: new mongodb_2.ObjectId(id) }, {
            $inc: { views: 1 },
        });
    }
