...
export class SEOReplacementService {
  private tokens: Partial<SEOTokens>;
   ...
   */
  private extractTokensFromSiteData(siteData: any): Partial<SEOTokens> {
    return {
      // Company Information - From next.config.js company object
      COMPANY_NAME:
        siteData.company?.name || siteData.companyName || "Homely Real Estate",
      COMPANY_TAGLINE:
        siteData.company?.tagline || siteData.tagline || "Find Your Dream Home",
      COMPANY_DESCRIPTION:
        siteData.company?.description ||
        `Professional real estate services by ${
          siteData.company?.name || siteData.companyName || "Homely Real Estate"
      ...
      FACEBOOK_URL:
        siteData.social?.facebook || "https://facebook.com/homelyrealestate",
      TWITTER_URL:
        siteData.social?.twitter || "https://twitter.com/homelyrealestate",
      INSTAGRAM_URL:
        siteData.social?.instagram || "https://instagram.com/homelyrealestate",
      LINKEDIN_URL:
      ...
      FACEBOOK_VERIFICATION_CODE:
        siteData.verification?.facebookVerification || "",
   ...
   */
  public replaceTokens(content: string): string {
   ...
   */
  public async replaceTokensInFile(
    filePath: string,
    fileContent: string
  ): Promise<string> {
   ...
   */
  public generateEnvVariables(): string {
...
`;
  ...
  public generateSEOReport(): {
    ...
    const completedTokens = allTokenKeys.filter(
      (key) =>
        this.tokens[key as keyof SEOTokens] &&
        String(this.tokens[key as keyof SEOTokens]).trim() !== ""
    );

    const missingTokens = allTokenKeys.filter(
      (key) =>
        !this.tokens[key as keyof SEOTokens] ||
        String(this.tokens[key as keyof SEOTokens]).trim() === ""
    );
    ...
    if (missingTokens.includes("COMPANY_DESCRIPTION")) {
      recommendations.push(
        "Add a compelling company description for better SEO"
      );
    }
    ...
    if (
      !this.tokens.FACEBOOK_URL &&
      !this.tokens.TWITTER_URL &&
      !this.tokens.INSTAGRAM_URL
    ) {
      recommendations.push(
        "Add social media profiles to increase online presence"
      );
    }
   ...
   */
  public getTokens(): Partial<SEOTokens> {
    return { ...this.tokens };
  }
   ...
   */
  public updateTokens(updates: Partial<SEOTokens>): void {
 ...
 */
export const createSEOReplacementService = (siteData: any) => {
...
export const DEFAULT_SEO_CONFIG = {
  ...
  COMPANY_DESCRIPTION:
    "Professional real estate services to help you find your perfect home",
